...
sub mapearArquivoNomes{
       ...
       foreach $row (0..@discoverNames-1){  
            ...
            if(length($palavra) > 0 and $i > 0 ){
               $informacao = Informacao->new( termo => $palavra, posicao => $row);      
               $informacao->indexSinonimos($string);           
               $hashMapeamentoArqNames{$informacao->termo} = $informacao;              
            }                            
   }
   $end_run = time();
   $run_time = $end_run - $start_run;
   ...
   foreach my $row (0..@discoverData-1){    
        $nomeDocumento = $discoverData[$row][0];
         ...
         foreach my $value (sort values %parametroHashArqNames){             
             ...
             if(length($value->indexSinonimos) > 0){
                print "    # Documento linha: $num  P.S.S.  = ".$value->termo."       ";
                usleep(100000);
                print ("\b" x (length("                                                                             ")));  
                print ("\b" x (length("    # Documento linha: $num  P.S.S.  = ".$value->termo."       ")));       
                $informacao = Informacao->new( termo => $value->termo, posicao => $value->posicao);
                $informacao->valor($discoverData[$row][$value->posicao]);
                $informacao->nomeDocumento($nomeDocumento);                
                $informacao->indexSinonimos($value->indexSinonimos);
                $informacao->posicaoDocumento($row);

                $hashOfHash{$nomeDocumento}{$value->termo}{$value->posicao}   =  $informacao;
             }
         }       
...
sub alterarArquivoData{
   ...
   foreach my $docNome (sort keys  %hashRef) {#Nome do documento
       ...
       foreach my $termo (sort keys %{ $hashRef{$docNome} }) {# Termo
          ...
          foreach my $posicao (sort keys %{ $hashRef{$docNome}{$termo} }) {# Posição
              my $posicaoTermoColuna = $hashRef{$docNome}{$termo}{$posicao}->posicao;
              my $string = $hashRef{$docNome}{$termo}{$posicao}->indexSinonimos;
              my $posicaoLinhaDocumento = $hashRef{$docNome}{$termo}{$posicao}->posicaoDocumento;
              my $valorTermo = $hashRef{$docNome}{$termo}{$posicao}->valor;
              if(length($string) > 0){                
   ...
   foreach $row (0..@discoverData-1){    
      ...
      print  $meuArquivo removerFlags($linha)."\n"
   }
...
sub identificarSinonimosDeSinonimos{
       ...
       foreach my $chave (sort keys %copiaHashMapeado){ 
            if (exists $novoHashMapeado{$chave}){  
                  #$palavra = extrairNomes($discoverNames[$row][$column]); 
                  $palavra = $novoHashMapeado{$chave}->termo;
                  $num = $num + 1;
                  print "  ->  $num - $palavra                ";
                  usleep(100000);
                  print ("\b" x (length("                                                                             ")));  
                  ...
                  if(length($palavra) > 1){
                     ...
                     if($i > 0 ){
                        foreach $key (sort keys %hashList){                                      
                           ...
                           # e marco a key que usei p/ busca no meuDiscover.names. Por exemplo [student] encontrei no arquivo. 
                            foreach $otherRow (0..@discoverNames-1){                                                              
                                 ...
                                 if(($key eq $palavraArquivo) and ($palavra ne $key)){
                                     ...
                                     #sinônimos.
                                     my $herdeiro =  $novoHashMapeado{$key}->indexSinonimos;
                         ...
                         $novoHashMapeado{$palavra}->indexSinonimos($string);
